...
/* ── Reveal animation — progressive enhancement ────────────────
   BUG FIX: Default state is VISIBLE (no JS = no hiding).
   Only when html.js class is present (added by JS immediately on load)
   do elements start hidden so they can be revealed on scroll.
   This ensures content is always accessible without JavaScript.
──────────────────────────────────────────────────────────────── */

/* Without JS: reveal elements are fully visible — no styles needed */

/* With JS active: hide elements until scrolled into view */
html.js .reveal {
  opacity: 0;
...
/* Revealed state — works both with and without html.js */
...
/* ══════════════════════════════════════════════════════════════════
   RESPONSIVE — Mobile  (BUG FIX: hero + nav)
══════════════════════════════════════════════════════════════════ */
@media (max-width: 768px) {
  ...
  /* ── Nav mobile — logo + hamburger fit without overlap ── */
  .hamburger { display: flex; }

  /* Tighten nav-inner gap so logo and hamburger share space cleanly */
  .nav-inner {
    gap: var(--space-sm);
    padding: 0 1.25rem; /* consistent mobile padding */
  }

  /* Let logo grow to fill space, push hamburger to edge */
  .nav-logo {
    flex: 1 1 auto;
    min-width: 0;
  }

  /* Slightly reduce logo height on mobile */
  .logo-img {
    height: 36px;
  }

  /* Hide desktop CTA pill — hamburger only on mobile */
  ...
  /* ── Hero mobile — proper cover + all content visible ── */
  .hero {
    min-height: 100svh;
    min-height: 100vh;
    align-items: flex-end; /* anchor content toward middle-bottom on mobile */
  }

  /* Focal point: keep the desk/room scene visible on mobile crop */
  .hero-img {
    object-position: 60% center;
  }

  /* Stronger left-side darkening overlay for text legibility on mobile */
  .hero-image-overlay {
    background:
      linear-gradient(to right, rgba(14,58,68,0.95) 0%, rgba(14,58,68,0.85) 55%, rgba(14,58,68,0.5) 100%),
      linear-gradient(to bottom, rgba(14,58,68,0.3) 0%, transparent 25%, transparent 65%, rgba(14,58,68,0.55) 100%);
  }

  /* Hero content: full width, padded away from nav */
  .hero-content {
    padding: calc(var(--nav-h) + 3rem) 1.25rem 3.5rem;
    max-width: 100%;
    align-self: auto; /* reset flex-end alignment for content */
  }

  /* Hero headline: tighter size for mobile */
  .hero-headline {
    font-size: clamp(2rem, 8vw, 3rem);
  }

  /* Hero subtext: full width, readable on small screens */
  .hero-sub {
    font-size: var(--text-base);
    max-width: 100%;
...
/* ── Reduced motion: disable all animations incl. reveal ── */
