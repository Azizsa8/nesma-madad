...
.site-nav:not(.scrolled) .nav-links li:not(:last-child)::after {
...
══════════════════════════════════════════════════════════════════ */
/* ── Solutions Redesign (Screenshot Layout) ── */
.solutions-header {
  background: var(--teal);
  position: relative;
...
.solutions-header .header-rule {
  width: 320px;
  height: 1px;
  background: rgba(255,255,255,0.3);
  margin-top: 2rem;
}
.solutions-header-curve {
  position: absolute;
  bottom: 0;
  left: 0;
  width: 100%;
...
.solution-row .row-text {
  flex: 1;
}
.solution-row .row-image {
  flex: 1;
}
.row-category-label {
  font-family: var(--font-display);
...
.solution-row:hover .row-img {
  transform: translateY(-8px);
}

/* Row alternate ordering on desktop */
.row-reverse .solution-row-inner {
...
.solutions-cta {
  background: var(--teal);
  background-image: var(--grad-sweep);
  padding: 6rem 0;
  text-align: center;
  color: var(--white);
}
